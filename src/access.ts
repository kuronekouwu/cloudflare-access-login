...
export class CloudflreAccess {
    ...
    private userAgent: string = 'cloudflared/2023.3.1';
    ...
    async login(url: string) {
        ...
        if (typeof kid === 'string') throw new Error('Cannot get \'kid\' in query')
        ...
        this.events.removeAllListeners('login');
        ...
        for (let i = 0; i < this.user_retry; i++) {
            try {
                ...
                const result = decryptData(
                    token.data,
                    token.headers['service-public-key'] || '',
                    secretKey,
                );
                if (!result) throw new Error('Get token failed. Please try again');
                return JSON.parse(Buffer.from(result).toString());
            } catch (e) {
                if (retry >= this.user_retry) {
                    throw new Error('Timeout of request to cloudflare access');
                }
